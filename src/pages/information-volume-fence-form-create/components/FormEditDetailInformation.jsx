...
import {Box, Button, Container, MenuItem, Snackbar, Stack, TextField, Typography, Fab} from "@mui/material";
...
export default function FormEditDetailInformation(obj) {

    const globalHooks = useContext(AppContext);

    const waterObject = obj.listDetailsTable[obj.count][0];

    const [typeWaterObject, setTypeWaterObject] = useState(waterObject.typeWaterObject);

    const [waterQualityCategory, setWaterQualityCategory] = useState(waterObject.waterQualityCategory);

    const [waterIntakeNumber, setWaterIntakeNumber] = useState(waterObject.waterIntakeNumber);

    const [northernLatitudeDegrees, setNorthernLatitudeDegrees] = useState(waterObject.northernLatitudeDegrees);
    const [northernLatitudeMinutes, setNorthernLatitudeMinutes] = useState(waterObject.northernLatitudeMinutes);
    const [northernLatitudeSeconds, setNorthernLatitudeSeconds] = useState(waterObject.northernLatitudeSeconds);

    const [easternLongitudeDegrees, setEasternLongitudeDegrees] = useState(waterObject.easternLongitudeDegrees);
    const [easternLongitudeMinutes, setEasternLongitudeMinutes] = useState(waterObject.easternLongitudeMinutes);
    const [easternLongitudeSeconds, setEasternLongitudeSeconds] = useState(waterObject.easternLongitudeSeconds);

    const [purposeWaterUse, setPurposeWaterUse] = useState(waterObject.purposeWaterUse);

    const [volumePermissibleFence, setVolumePermissibleFence] = useState(waterObject.volumePermissibleFence);

    const [fullVolume, setFullVolume] = useState(waterObject.fullVolume);

    const [firstMonth, setFirstMonth] = useState(waterObject.firstMonth);
    const [secondMonth, setSecondMonth] = useState(waterObject.secondMonth);
    const [thirdMonth, setThirdMonth] = useState(waterObject.thirdMonth);
    ...
    const handleOpen = () => {
        ...
        globalHooks.setNameWaterObject({
            code: waterObject.codeWaterObject,
            name: waterObject.nameWaterObject
        })
    };
    const handleClose = () => {
        setOpen(false);
    ...
    function handlerAddDetails() {
        ...
        const temp = [...obj.listDetailsTable];
        temp[obj.count] = newObj;
        obj.setListDetailsTable(temp);
        handleClose();
    }
